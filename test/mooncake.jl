...
@timedtestset "EIG AD Rules with eltype $T" for T in ETs
    ...
    @testset for alg in (
        ...
        @testset "eig_full" begin
            Mooncake.TestUtils.test_rule(rng, eig_full, A, alg; output_tangent = dDV, atol = atol, rtol = rtol)
            test_pullbacks_match(rng, eig_full!, eig_full, A, (D, V), (ΔD2, ΔV), alg)
        end
        @testset "eig_vals" begin
            Mooncake.TestUtils.test_rule(rng, eig_vals, A, alg; atol = atol, rtol = rtol)
...
MatrixAlgebraKit.copy_input(::typeof(copy_eigh_trunc_no_error), A) = MatrixAlgebraKit.copy_input(eigh_trunc, A)
    ...
    @testset "size ($m, $n)" for n in (17, m, 23)
        ...
        @testset for alg in (
            ...
            @testset "svd_compact" begin
                ...
                Mooncake.TestUtils.test_rule(rng, svd_compact, A, alg; output_tangent = dUSVᴴ, atol = atol, rtol = rtol)
            ...
            @testset "svd_full" begin
                ...
                Mooncake.TestUtils.test_rule(rng, svd_full, A, alg; output_tangent = dUSVᴴ, atol = atol, rtol = rtol)
                test_pullbacks_match(rng, svd_full!, svd_full, A, (U, S, Vᴴ), (ΔUfull, ΔSfull, ΔVᴴfull), alg)
            end
            @testset "svd_vals" begin
                Mooncake.TestUtils.test_rule(rng, svd_vals, A, alg; atol = atol, rtol = rtol)
                S = svd_vals(A, alg)
            ...
            @testset "svd_trunc" begin
                @testset for r in 1:4:minmn
                    ...
                    test_pullbacks_match(rng, svd_trunc_no_error!, svd_trunc_no_error, A, (U, S, Vᴴ), (ΔU, ΔS2, ΔVᴴ), truncalg)
                end
...
@timedtestset "Polar AD Rules with eltype $T" for T in ETs
    ...
    @testset "size ($m, $n)" for n in (17, m, 23)
        ...
        @testset for alg in PolarViaSVD.(
            ...
            if m >= n
                WP = left_polar(A, alg)
                Mooncake.TestUtils.test_rule(rng, left_polar, A, alg; atol = atol, rtol = rtol)
                test_pullbacks_match(rng, left_polar!, left_polar, A, WP, (randn(rng, T, m, n), randn(rng, T, n, n)), alg)
            elseif m <= n
                PWᴴ = right_polar(A, alg)
                Mooncake.TestUtils.test_rule(rng, right_polar, A, alg; atol = atol, rtol = rtol)
...
@timedtestset "Orth and null with eltype $T" for T in ETs
    ...
    @testset "size ($m, $n)" for n in (17, m, 23)
        ...
        VC = left_orth(A)
        CVᴴ = right_orth(A)
        Mooncake.TestUtils.test_rule(rng, left_orth, A; atol = atol, rtol = rtol, is_primitive = false)
        test_pullbacks_match(rng, left_orth!, left_orth, A, VC, (randn(rng, T, size(VC[1])...), randn(rng, T, size(VC[2])...)))
        Mooncake.TestUtils.test_rule(rng, right_orth, A; atol = atol, rtol = rtol, is_primitive = false)
        test_pullbacks_match(rng, right_orth!, right_orth, A, CVᴴ, (randn(rng, T, size(CVᴴ[1])...), randn(rng, T, size(CVᴴ[2])...)))

        Mooncake.TestUtils.test_rule(rng, left_orth_qr, A; atol = atol, rtol = rtol, is_primitive = false)
        test_pullbacks_match(rng, ((X, VC) -> left_orth!(X, VC; alg = :qr)), left_orth_qr, A, VC, (randn(rng, T, size(VC[1])...), randn(rng, T, size(VC[2])...)))
        if m >= n
            Mooncake.TestUtils.test_rule(rng, left_orth_polar, A; atol = atol, rtol = rtol, is_primitive = false)
        ...
        Mooncake.TestUtils.test_rule(rng, left_null_qr, A; atol = atol, rtol = rtol, is_primitive = false, output_tangent = dN)
        test_pullbacks_match(rng, ((X, N) -> left_null!(X, N; alg = :qr)), left_null_qr, A, N, ΔN)

        Mooncake.TestUtils.test_rule(rng, right_orth_lq, A; atol = atol, rtol = rtol, is_primitive = false)
        test_pullbacks_match(rng, ((X, CVᴴ) -> right_orth!(X, CVᴴ; alg = :lq)), right_orth_lq, A, CVᴴ, (randn(rng, T, size(CVᴴ[1])...), randn(rng, T, size(CVᴴ[2])...)))

        if m <= n
            Mooncake.TestUtils.test_rule(rng, right_orth_polar, A; atol = atol, rtol = rtol, is_primitive = false)
        ...
        dNᴴ = make_mooncake_tangent(ΔNᴴ)
        Mooncake.TestUtils.test_rule(rng, right_null_lq, A; atol = atol, rtol = rtol, is_primitive = false, output_tangent = dNᴴ)
