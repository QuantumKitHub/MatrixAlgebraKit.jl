...
# CUDA tests
# ----------
# a general dense matrix is supported on device through the native `MatrixFunctionViaTaylor`
if CUDA.functional()
    for T in BLASFloats
        TestSuite.seed_rng!(123)
        CUDA_EIGH_ALGS = (MatrixFunctionViaEigh(Jacobi()), MatrixFunctionViaEigh(DivideAndConquer()))
        TestSuite.test_exponential_algs(CuMatrix{T}, (m, m), (MatrixFunctionViaTaylor(),))
...
if AMDGPU.functional()
    for T in BLASFloats
        ...
        TestSuite.test_exponential_algs(ROCMatrix{T}, (m, m), (MatrixFunctionViaTaylor(),))
